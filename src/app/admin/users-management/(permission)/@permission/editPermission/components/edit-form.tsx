"use client";
import { editPermission } from "@/app/actions/permission";
...
export type PermissionEditFormValue = {
  permissionId: string;
  name: string;
  entityId: string;
  action: Action[];
...
export default function PremissionEditFrom({
  permission,
  entities,
}: {
  ...
  const {
    ...
    formState: { errors },
    setValue,
  } = useForm<PermissionEditFormValue>();

  const { toast } = useToast();
  const [actions, setActions] = useState<Action[]>([...permission.action]);
  ...
  const onSubmit = handleSubmit(async (data: PermissionEditFormValue, e) => {
  ...
  useEffect(() => {
    setValue("entityId", permission.entityId);
  }, []);
  return (
        ...
        <CardContent>
          <form className="my-5 flex flex-col gap-5">
            ...
            <div className="grid grid-cols-3">
              ...
              <Select
                defaultValue={permission.entityId}
                {...register("entityId", {
