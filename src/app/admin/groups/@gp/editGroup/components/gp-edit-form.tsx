"use client";
import { addGroup, editGroup } from "@/app/actions/group";
...
export type GroupEditFormValue = {
  gpId: string;
  name: string;
...
export default function GroupEditForm({
  ...
  group: Group & { permissions: Permission[] };
  ...
  const {
    ...
    formState: { errors },
    setValue,
  } = useForm<GroupEditFormValue>();
  const [checkList, setCheckList] = React.useState<string[]>(
    group.permissions.map((p) => p.id),
  );
  const { toast } = useToast();

  const onSubmit = handleSubmit(async (data: GroupEditFormValue, e) => {
    data.gpId = group.id;
    ...
    try {
      ...
      toast({ title: "Oh! Something went wrong", description: error.message });
    }
    router.back();
  });
          ...
          <form className="my-5 flex flex-col gap-5">
            ...
            <div className="grid grid-cols-3">
              ...
              <Textarea
                ...
                defaultValue={group.description?.toString()}
              />
            </div>
            <div className="grid grid-cols-3">
              <Label
